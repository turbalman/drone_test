...
if [[ $DRONE_SOURCE_BRANCH == "main" ]] && [[ $DRONE_TARGET_BRANCH == "main" ]];then
  if [[ $DRONE_BUILD_KEY == "pipeline_1" ]];then
      echo "pipeline_1->code_duplication_check.sh started!"
      echo "pipeline_1->code_duplication_check.sh finished!"
  elif [[ $DRONE_BUILD_KEY == "pipeline_2" ]];then
      echo "pipeline_2->code_duplication_check.sh started!"
      echo "pipeline_2->code_duplication_check.sh finished!"
  else
